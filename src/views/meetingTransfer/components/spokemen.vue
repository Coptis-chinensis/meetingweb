<template>
  <div class="spoke">
    ...
    <el-checkbox-group :disabled="isVoicePrint"
      ...
      <div title="双击切换说话人" class="spokemanCard" v-for="(spokemen,index) in spokeMen[0].spokeInfo" :key="index">
        <!-- 其他说话人 -->
        <div v-if="spokemen.userStatus == 1" @dblclick="changeSpk(spokemen)">
          <div class="yuan">
            <div class="firstName otherSpoke">
              <span>{{spokemen.spokeMan.substring(0,1)}}</span>
            </div>
          </div>
          <div class="allName">
            ...
            <el-input :disabled="isVoicePrint"
                      ...
                      v-model="spokemen.spokeMan"
                      @change="changeSpokeman1(index)"></el-input>
            <div v-if="!isVoicePrint" class="xiala" id="xiala">
              <div v-show="spokemen.attendeShow" class="attendeCard"
        ...
        <!-- 当前说话人 -->
        <div v-if="spokemen.userStatus == 0">
          <div class="yuan">
            <div class="firstName nowSpoke" style="background-color: #4359C3">
              <span>{{spokemen.spokeMan.substring(0,1)}}</span>
            </div>
          </div>
        ...
        <i v-if="spokemen.userStatus == 1 && !isVoicePrint" class="el-icon-remove-outline" @click="delSpokeman(spokemen)"></i><br>
      </div>
    </el-checkbox-group>
    <!-- 创建声纹人员按钮 -->
    <div class="spokemanCard" v-if="spokeMen[0].length < 8 && !isVoicePrint" @click="addSpokeman">
      <div class="yuan">
        <div style="font-weight: 100;" class="firstName addName">
          <span>+</span>
        </div>
      </div>
      <div class="allName">
...
export default {
  ...
  watch:{
    'spokeMen': function () {
      if(!this.checkAll) { // 全选
        this.checkedCities = []
        for(let i = 0; i < this.spokeMen[0].spokeInfo.length; i++) {
          this.checkedCities.push(i)
          this.chooseSpk.push(this.spokeMen[0].spokeInfo[i].spokeMan)
        }
      }
    },
  },
  ...
  methods:{
    ...
    handleCheckedCitiesChange(value) {
      ...
      for(let i = 0; i < value.length; i++) {
        this.chooseSpk.push(this.spokeMen[0].spokeInfo[value[i]].spokeMan)
      }
      // console.log(this.chooseSpk);
      this.chooseContent()
      let checkedCount = value.length;
      this.checkAll = (checkedCount < this.spokeMen[0].spokeInfo.length);
      this.isIndeterminate = checkedCount > 0 && checkedCount < this.spokeMen[0].spokeInfo.length;
    },
    ...
    // 焦点了input框
    inputFocus(index) {
      var spokemen = this.spokeMen[0].spokeInfo
      spokemen[index].attendeShow = true
      this.$set(spokemen, index, spokemen[index]);
    },

    // 离开了input框
    inputBlur(index) {
      var spokemen = this.spokeMen[0].spokeInfo
      spokemen[index].attendeShow = false
      // 为了让用户可以点击下拉框中的内容
      setTimeout(() => {
        this.$set(spokemen, index, spokemen[index]);
    ...
    chooseAttende(item, index){
      ...
      this.changeSpokeman(item, index)
    },
    ...
    // 删除声纹人员
    delSpokeman(item){
      ask_deleteSpokeman(item.id).then(res => {
        if (res.success) {
    ...
    // 切换说话人名称
    changeSpokeman(item, index){
      var form = new FormData()
      var spokemen = this.spokeMen[0].spokeInfo
      console.log(spokemen[index].id, "???");
      form.append("meetingID", this.meetingId)
      form.append("id", spokemen[index].id)
      form.append("userStatus", spokemen[index].userStatus)
      form.append("channelID", this.spokeMen[0].channelID)
      form.append("spokeMan", spokemen[index].spokeMan)
    ...
    // 修改说话人名称
    changeSpokeman1(index){
      var spokeman = this.spokeMen[0]
      var form = new FormData()
      console.log(spokeman.spokeInfo[index].id, "???");
      form.append("meetingID", this.meetingId)
      form.append("id", spokeman.spokeInfo[index].id)
      form.append("userStatus", spokeman.spokeInfo[index].userStatus)
      form.append("channelID", spokeman.channelID)
      form.append("spokeMan", spokeman.spokeInfo[index].spokeMan)
    ...
    changeSpk(item){
      ...
      var form = new FormData()
      form.append("meetingID", this.meetingId)
      form.append("channelID", this.channel)
      form.append("id", item.id)
      ask_changeSpokeMan(form).then(res =>{
        if (res.success) {
          this.$emit('getMicName', 'getMicName')
        }
      })
